...
#------------------------------------------
trainFeatures = io.loadh('trainFeatures2.h5')['arr_0']
testFeatures = io.loadh('testFeatures2.h5')['arr_0']
...
def train_nn():
    ...
    np.savetxt('trained_theta2.txt',theta)
    return theta
train_nn()
#-----------------------------------------------------------------------------------------------
...
#    pred = np.rint(pred)
    ...
    return pred
#theta = train_nn()
try: theta = np.loadtxt('trained_theta2.txt')
except: theta = train_nn()
p = softmaxPredict(theta, trainFeatures.T)
print "train prediction accuracy for featured data: %f percent" %(np.mean(p == trainLabels.ravel())*100)
p = softmaxPredict(theta, testFeatures.T)
print "test prediction accuracy for featured data: %f percent" %(np.mean(p == testLabels.ravel())*100)
